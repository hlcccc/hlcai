...
class BioMedLlamaModel(BaseModel):
    """Hugging Face Model."""
    ...
    def __init__(self, model_name, stop_sequences=None, max_new_tokens=None):
        ...
        self.stop_sequences = stop_sequences + [self.tokenizer.eos_token, '<|eot_id|>'] 
        self.token_limit = context_len
        self.device = 'cuda'
    
    def apply_chat_template(self, msgs, image=None, system_prompt=''):
        copy_msgs = msgs
    ...
    def process_input(self, prompt, image_path=None):
        ...
        # res = self.model.chat(image=image, msgs=conv, tokenizer=self.tokenizer, sampling=True, temperature=0.95, stream=True)

        msgs, images = self.apply_chat_template(conv, image, system_prompt="A chat between a curious human and an artificial intelligence assistant. The assistant gives trustworthy answers to the user's questions.")
        processed_prompt = self.processor.tokenizer.apply_chat_template(msgs, tokenize=False, add_generation_prompt=True)
        inputs = self.processor(processed_prompt, images, return_tensors="pt", max_length=self.token_limit).to(self.device)
